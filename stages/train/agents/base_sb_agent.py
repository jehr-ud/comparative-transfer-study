...
def train_sb_agent(
    ...
    episode_rewards = (
        base_env.get_episode_rewards()
        if hasattr(base_env, "get_episode_rewards")
        else []
    )

    mean_reward = (
        np.mean(episode_rewards[-100:]) if episode_rewards else None
    )

    print(
        f"[{model_name}-{difficulty}] "
        f"Average reward (last 100 episodes): {mean_reward}"
    )
