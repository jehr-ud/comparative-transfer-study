    ...
    def get_local_prediction(self, current_pos, valid_actions, prev_pos):
        ...
        for action in valid_actions:
            ...
            activation_score = -float('inf') if neighbor_pos == prev_pos \
                else downstream_activity[
                    0,
                    self._pos_to_idx(neighbor_pos)
                ].item()
        ...
        return best_action if best_action != -1 else random.choice(
            valid_actions
        )
...
def calculate_fingerprint_distance(fp1, fp2):
    """Calculate the Euclidean distance between two fingerprints."""
...
class PrefrontalCortex:
    ...
    def __init__(self, atlas_path="snn_atlas.pkl"):
        ...
        self.active_agent = None  # The agent ready for the current task
        print(
    ...
    def handle_task(self, maze_name, env_info, identification_episodes=30):
        """
        Train an agent for a new task using the full hierarchy:
        Recognition, Adaptation (Fine-Tuning), or Learning from Scratch.
        """
        print(f"\n--- HANDLING NEW TASK: Maze '{maze_name}' ---")

        # 1. Identification Phase with a diagnostic agent
        diag_agent = AdapSNNAgent(
            name=f"diag_{maze_name}",
            difficulty=env_info['env'].size,
            env_info=env_info
        )
        print(f"Starting identification phase ({identification_episodes}")
        ...
        # 2. Strategic Decision Hierarchy
        # Level 1: Accurate Recognition
        if fingerprint in self.cerebral_atlas:
            model_path = self.cerebral_atlas[fingerprint]
            print(f"CONTEXT FOUND! Loading expert brain from:{model_path}")
        ...
        if self.cerebral_atlas:
            ...
            print("NEW CONTEXT.")
            print(f"Most similar brain found (Dist: {min_distance:.2f}).")
            print(f"Performing 'brain transplant' from: {model_path}")

            # We re-instantiate the agent with the name of the *new* maze
            # but load the *donor* brain.
            self.active_agent = AdapSNNAgent(
        ...
        # Level 3: Learning from Scratch
        print("Atlas is empty or no similar brain found..")
        print("Preparing for learning from scratch..")
        self.active_agent = AdapSNNAgent(
            name=f"agent_{maze_name}",
            difficulty=env_info['env'].size,
            env_info=env_info
    ...
    def train_active_agent(self, maze_name, training_episodes=200):
        """
        Runs the full training loop for the
        ...
        for i in range(training_episodes):
            current_epsilon = max(
                min_epsilon,
                initial_epsilon * (epsilon_decay_rate ** i)
            )
        ...
        # Save the newly trained/adapted brain and update the Atlas
    ...
    def predict(self, obs, **kwargs):
        """Delegate the prediction call to the active agent."""
        if self.active_agent is None:
            raise RuntimeError(
                "No active agent. Call handle_task to prepare an agent."
            )
        return self.active_agent.predict(obs)

    def reset(self):
        """Resets the state of the active agent."""
